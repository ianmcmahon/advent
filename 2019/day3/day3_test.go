...
func TestExamples(t *testing.T) {
	for _, tc := range testCases {
		pathA, err := asSegments(tc.pathA)
		if err != nil {
			fmt.Printf("%v\n", err)
			t.Fail()
		}
		pathB, err := asSegments(tc.pathB)
		if err != nil {
			fmt.Printf("%v\n", err)
			t.Fail()
		}
		...
		min := findMinDistance(intersections)
		fmt.Printf("min crossing distance: %d\n", min)
		if min != tc.expected {
			t.Error()
		}
		var bestIntersection image.Point
		bestSteps := 65536
		for _, pt := range intersections {
			aSteps := stepsToPoint(pathA, pt)
			bSteps := stepsToPoint(pathB, pt)
			if aSteps+bSteps < bestSteps {
				bestSteps = aSteps + bSteps
				bestIntersection = pt
			}
		}
		fmt.Printf("Best steps is %d at intersection %v\n", bestSteps, bestIntersection)
		fmt.Println()
	}
}

func TestPart1(t *testing.T) {
	pathA, err := asSegments(part1.pathA)
	if err != nil {
		fmt.Printf("%v\n", err)
		t.Fail()
	}
	pathB, err := asSegments(part1.pathB)
	if err != nil {
		fmt.Printf("%v\n", err)
		t.Fail()
	}
